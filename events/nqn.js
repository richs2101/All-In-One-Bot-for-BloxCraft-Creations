...
// Cache for NQN configurations to reduce database queries
...
async function loadNqnConfig() {
    ...
    try {
        const configs = await NqnConfig.find({ status: true }); // Only load enabled configs
        ...
        return nqnConfigCache; // Return existing cache on error
    }
}
...
module.exports = async (client) => {
    ...
    // Event handler for when NQN config is updated via command
